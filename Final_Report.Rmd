...
```{r finish race, echo=FALSE, fig.height=4}
df_data %>% 
...
```{r warning=FALSE, fig.width=16, fig.height=10, echo=FALSE}
#df_validate_pos %>%
#    add_uncertainties(f_driv_pos, interval = "prediction", prefix = "pi")
...
```{r fig.width = 20,fig.height=15, echo=FALSE, fig.align="center"}
df_cleanLapTimes %>%
  filter(year >= 2014)%>%
  mutate(driver_name = fct_reorder(as.character(driver_name),positionOrder))%>%
  ggplot(mapping = aes(x = driver_name, y = positionOrder)) +
...
```{r warning=FALSE, fig.height=4, echo=FALSE, fig.align="center"}
df_preds <-
  df_validate_pos %>%
  add_predictions(f_driv_pos) %>%
  mutate(pred_driv = pred) %>%
...
```{r warning=FALSE, fig.width=16, fig.height=13, echo=FALSE, fig.align="center"}
#df_validate_pos %>%
#    add_uncertainties(f_driv_pos, interval = "prediction", prefix = "pi")
