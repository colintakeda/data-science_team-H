...
df_timedata <- read_csv("processed_data/std_avg_laptime.csv") %>% filter(year >= 2014)
df_data <- read_csv("processed_data/clean_f1.csv") %>% filter(year >= 2014)
```
...
df_pred_drivcons %>%
  ggplot() + 
  geom_point(aes(x=positionOrder, y=pred)) +
  geom_point(data = df_pred_driv, aes(x=positionOrder, y=pred, color = 'r')) +
  geom_point(data = df_pred_cons, aes(x=positionOrder, y=pred, color = 'b')) +
